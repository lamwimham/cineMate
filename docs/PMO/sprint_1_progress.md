...
| 5 | Demo 准备完成 | ✅ | `scripts/demo_day5.py`, Mock Agent 演示 |
| 5 | 20 Case 测试 | ✅ | DAG 依赖/Worker 异步/Event 触发 |
| 5 | Sprint Review Demo | ⏳ | 17:00 演示 |

**当前**: Demo 准备完成 ✅，等待 Sprint Review
**分支**: 已合并到 main
**阻塞**: 无

**Demo 交付物**:
- `scripts/demo_day5.py` (支持 `--real-agent` 扩展)
- `scripts/run_demo.sh` (一键启动)
- `docs/PMO/demo_day5_guide.md` (演示指南)

**验收确认**:
- ✅ Agent 意图解析 (模拟 LLM 输出 DAG JSON)
- ✅ Toolkit 注册 (EngineTools 挂载完毕)
- ✅ Engine 依赖控制 (DAG 拓扑构建与脏节点分析)
- ✅ Queue → Worker 链路 (RQ 成功派发，Worker 执行并返回)
- ✅ Event-Driven 触发 (node_completed 自动推进下一节点)
...
| **Day 5** | **14:00** | ✅ **hermes Demo 准备完成** | **hermes** | **Mock Agent 演示就绪** |
